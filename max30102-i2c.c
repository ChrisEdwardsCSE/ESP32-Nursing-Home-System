...
/**
 * Configure sampling stuff. go for 100 samples/s and 15 bit resolution
 */
void max30102_config(i2c_master_dev_handle_t master_dev_handle)
{
#if MAX_DEBUG
...
void max30102_fifo_read(i2c_master_dev_handle_t master_dev_handle, uint32_t* hr_sample, uint32_t* ir_sample)
{
    printf("&hr_sample %p, ir_sample %p\n", hr_sample, ir_sample);
    ...
    *ir_sample = (uint32_t)((rx_data_buf[0] << 16) | (rx_data_buf[1] << 8) | (rx_data_buf[2])) & 0x3ffff;
    *hr_sample = (uint32_t)((rx_data_buf[3] << 16) | (rx_data_buf[4] << 8) | (rx_data_buf[5])) & 0x3ffff;
}